...
def possibilities_grid(current_state, action, values, discount=0.9, second_problem=False):
    '''
    ...
    for (l1_rentals, l1_returns), l1_proba in possibilities_l1.items():
        for (l2_rentals, l2_returns), l2_proba in possibilities_l2.items():
            ...
            action_charge = abs(action) * 2
            parking_charge = 0
            if second_problem:
                if action > 0:
                    action_charge = (abs(action) - 1) * 2
                parking_charge = sum((4 if cars > 10 else 0 for cars in state))
            reward = 10 * (l1_rentals + l2_rentals) - action_charge - parking_charge
            bellman_value += proba * (reward + discount * values[state])
    return bellman_value

def policy_iteration(model_filename, second_problem=False, discount=0.9, delta_thresh=0.001):
    ...
    while not stable:
        ...
        while True:
            ...
            for state in states:
                temp = values[state]
                values[state] = possibilities_grid(state, policy[state], values, second_problem=second_problem, discount=discount)
        ...
        for state in states:
            temp = policy[state]
            action_value = lambda action: possibilities_grid(state, action, values, second_problem=second_problem, discount=discount)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "policy":
        policy_iteration(sys.argv[2], second_problem=True)
    elif sys.argv[1] == "vis":
